...
renderer.setSize(innerWidth, innerHeight); renderer.setPixelRatio(Math.min(devicePixelRatio, 1.5)); renderer.shadowMap.enabled = true; renderer.shadowMap.type = T.PCFSoftShadowMap; renderer.shadowMap.autoUpdate = false; renderer.shadowMap.needsUpdate = true; renderer.toneMapping = T.ACESFilmicToneMapping; renderer.toneMappingExposure = 1.2; renderer.outputColorSpace = T.SRGBColorSpace;
...
function quality() { const high = economy.state.settings.quality === 'high'; renderer.setPixelRatio(Math.min(devicePixelRatio, high ? 1.5 : 1)); renderer.shadowMap.enabled = high; renderer.shadowMap.needsUpdate = true; atmosphere.rain.geometry.setDrawRange(0, high ? 3000 : 1100); }
...
player.onStep = () => sound.step(); ui.onStart = () => { if (economy.state.playtime < 1) time = 0; }; ui.onReset = () => { for (const c of city.constructions) city.root.remove(c.group); city.constructions = []; city.sync(true); player.teleport(0, 77); player.pitch = -.025; ui.tracked = 'scrap'; time = 0; sound.apply(); quality(); };
...
if (dev) {
  const api = { economy, city, player, atmosphere, ui, renderer, scene, camera, status: () => ({ crowns: economy.state.crowns, rate: economy.rate, stage: economy.stage, draws: renderer.info.render.calls, triangles: renderer.info.render.triangles, fps, locked: player.locked, position: player.position.toArray(), constructions: city.constructions.length }), stage: (level: number) => { for (const p of PROPERTIES) { economy.state.properties[p.id].level = level; economy.state.properties[p.id].automated = level > 1; } for (const i of INFRA) economy.state.infrastructure[i.id] = Math.min(3, Math.max(0, level - 1)); economy.state.crowns = 10000; if (level >= 3) economy.state.districts = ['canal', 'heights']; else economy.state.districts = []; city.sync(true); ui.render(); }, view: (name: string) => { const views: Record<string, [number, number, number, number?]> = { spawn: [0, 77, 0], street: [0, 26, 0], scrap: [-8, 37, Math.PI / 2], boiler: [7, 43, -Math.PI / 2], market: [1, -9, -.9], square: [0, -22, 0], roof: [-34, -56, Math.PI, 8.15], canal: [56, -6, -Math.PI / 2], foundry: [34, 21, .65], boilerYard: [35, 46, .8], housing: [-34, 32, Math.PI / 2], gate: [0, 59, Math.PI] }; const v = views[name]; if (v) player.teleport(...v); player.pitch = .05; } }; Object.assign(window, { __TERRA__: api });
}
