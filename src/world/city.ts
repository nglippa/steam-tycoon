...
export class City {
  housingFrontages:{x:number;z:number;width:number;height:number}[]=[];
  crowd!: CrowdBatch;
  ...
  buildGround() { const g = new T.Group(); this.root.add(g); box(g, 0, -.5, 0, 158, 1, 190, mats.dirt); box(g, 0, .018, 9, 19, .06, 139, mats.road);
    ...
    box(g, 44.5, -.14, -8, 8, .2, 144, mats.dark); const waterMat = new T.MeshStandardMaterial({ color: '#344c43', metalness: .55, roughness: .22, transparent: true, opacity: .82 }); this.water = new T.Mesh(new T.PlaneGeometry(8,142,12,90),waterMat);this.water.rotation.x=-Math.PI/2;this.water.position.set(44.5,.025,-8);this.root.add(this.water);
  ...
  buildBlocks() {
    ...
    for (const side of [-1, 1]) for (let i = 0; i < 6; i++) { const b = new T.Group(); b.position.set(side * 48, 0, 50 - i * 23); b.rotation.y = side < 0 ? Math.PI / 2 : -Math.PI / 2; if (side > 0) b.position.x = 68; const width=14+random()*3,height=12+random()*7;this.facade(b,width,height,10,i+1);if(side<0)this.housingFrontages.push({x:-42.95,z:b.position.z,width,height}); g.add(b); this.collider(b.position.x, b.position.z, 10, 17); }
  ...
  createPopulation() { for (let i = 0; i < 42; i++) { const npc = citizen([mats.rust, mats.teal, mats.wood, mats.cream, mats.red][i % 5], i);  this.root.add(npc.group); this.npcs.push(npc); }
  ...
  buildInfrastructure() { this.disposeGroup(this.infrastructure); const g = this.infrastructure; const s = this.economy.state.infrastructure;
    ...
    if (s.housing > 0) { for(const home of this.housingFrontages){const w=new T.Group();w.position.set(home.x,0,home.z);w.rotation.y=Math.PI/2;for(let x=-home.width/2+2;x<home.width/2-1;x+=3)for(let y=4.5;y<home.height-1.7;y+=3.5)windowUnit(w,x,y,0,true,1.35,2.3);g.add(w);} }
  ...
  update(dt: number, time: number) { this.presentation.update(dt,time); for (const p of PROPERTIES) { const v = this.properties.get(p.id)!; const level = this.economy.state.properties[p.id].level; v.gear.rotation.z -= dt * (.35 + level * .6); v.piston.position.y = .95 + Math.sin(time * (1 + level)) * .22; v.machine.rotation.z = level ? 0 : Math.sin(time * 9) * .008; }
