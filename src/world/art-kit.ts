...
export const artMats = {
  plaster: new T.MeshStandardMaterial({color:'#b8a788',roughness:.94}),
  fadedPaint: new T.MeshStandardMaterial({color:'#657a7b',roughness:.85}),
  coal: new T.MeshStandardMaterial({color:'#23272b',roughness:1}),
  mud: new T.MeshStandardMaterial({color:'#494237',roughness:.75}),
  ochre: new T.MeshStandardMaterial({color:'#cda051',roughness:.8}),
  wine: new T.MeshStandardMaterial({color:'#813f51',roughness:.85}),
  paper: new T.MeshStandardMaterial({color:'#c3b593',roughness:1}),
  furnace: new T.MeshBasicMaterial({color:'#ff942f'}),
  ember: new T.MeshBasicMaterial({color:'#ffd38a'}),
  pool: new T.MeshBasicMaterial({color:'#efa952',transparent:true,opacity:.1,depthWrite:false,side:T.DoubleSide}),
};
...
export function canopy(g:T.Object3D,x:number,y:number,z:number,w:number,d:number,mat:Material=mats.red) {
  const geo=new T.PlaneGeometry(w,d,8,4);const pos=geo.attributes.position;
  for(let i=0;i<pos.count;i++){const xx=pos.getX(i);const yy=pos.getY(i);pos.setXYZ(i,xx,Math.cos(xx/w*Math.PI)*.6+yy*.07,yy);}
  geo.computeVertexNormals();const mesh=new T.Mesh(geo,mat);mesh.position.set(x,y,z);g.add(mesh);
  for(const dx of [-w/2,w/2])for(const dz of [-d/2,d/2])cyl(g,x+dx,y/2,z+dz,.04,y,mats.iron);
  for(let dx=-w/2+.2;dx<w/2;dx+=.4)box(g,x+dx,y+Math.cos(dx/w*Math.PI)*.6,z+d/2,.32,.22,.045,mat);
  return mesh;
}
